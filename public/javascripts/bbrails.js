...
function cbAddColumn() {
    ...
    $('columnBrowser').innerHTML += "<div id=\"columnBrowserLevel" + cbNumColumns + "\" style=\"width: " + cbColWidth + "; height: " + cbColHeight + "; overflow: auto; float: left; border-width: " + cbBorderWidth + " " + cbBorderWidth + " " + cbBorderWidth + " 0; border-style: solid; border-color: gray;\">Loading...</div>";
...
function setupTextFieldHints() {
    textFieldHints._each(function (a) {
...
function changeGalleryImage(galleryName, index) {
    ...
    $$('.' + galleryName + '_thumb').each(function (el) {
        if (el.id == galleryName + '_thumb_' + index) {
            ...
            el.onmouseout = function () { el.style.borderColor = '#242424' };
        }
    });
...
function scanForPageObjects(page_id, parent_key, version) {
    ...
    // remove the cruft
    cmsPageObjects.each(function(pair) {
